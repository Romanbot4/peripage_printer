 ...
 */
class BluetoothHelper(private val appActivity: Activity, private val context: Context) {
    ...
    fun connectBluetoothDeviceMacAddress(address: String) : Boolean {
        return try {
            checkAndRequestBluetoothPermission()
            val remoteDevice = bluetoothAdapter?.getRemoteDevice(address) ?: return false
            val uuid = UUID.fromString("00001101-0000-1000-8000-00805f9b34fb")
    ...
    fun findBluetoothDevices(): Iterable<BluetoothDevice>? {
        checkAndRequestBluetoothPermission()
     ...
     */
    suspend fun printImage(bitmap: Bitmap, width: Int?): Boolean {
        val printWidth: Int = width ?: 384
        ...
        try {
            val scale = printWidth.toFloat() / bitmap.width
            ...
            val resizedBitmap = Bitmap.createScaledBitmap(grayBitmap, printWidth, height, true)
